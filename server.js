...
sequelize.sync().then(() => {
  ...
  app.use(
    morgan(
      ":method :url :status :reqdata :res[content-length] - :response-time ms"
    )
  );
  app.use(cors());
  ...
  app.get("/redirect-url", async (req, res) => {
    ...
    const result = ZaloPay.VerifyCheckSum(req.query);
    if (result == true) {
      let response = await ZaloPay.GetOrderStatus(req.query.apptransid);
